...
### LTO experiment (2026-06-19, issue #470) — no measurable gain, declined

Tested whether link-time optimisation inlines the remaining large cross-TU hot
symbols (`assimilation`, `compute_rates`). Two findings.

**Mechanism: `UseLTO: yes` is a no-op in our workflow.** (1) `make compile` uses
`pkgbuild::compile_dll()` → `R CMD SHLIB`, which never reads the `UseLTO`
DESCRIPTION field (only `R CMD INSTALL` does), so the benchmark loop always
measures a non-LTO build. (2) This R 4.6.0's `etc/Makeconf` has empty `LTO`,
`LTO_OPT`, `LTO_LD`, `LTO_FC` macros — built without `--enable-lto` — so even
`R CMD INSTALL .` produces a non-LTO build. To measure LTO at all required the
issue's non-preferred fallback: `-flto` on `PKG_CXXFLAGS`/`PKG_LIBS` in
`src/Makevars` (Apple clang 21 + ld64; objects compiled to `LLVM bitcode`,
confirming LTO engaged).

**Result: flat.** Same-session A/B, sample off, `PLANT_PROFILE_REPEATS=20`, two
runs each:

| metric | no-LTO baseline | `-flto` LTO |
|---|---:|---:|
| one-iter `scm` | 90.7 / 92.7 ms | 89.7 / 92.7 ms |
| one-iter `build_schedule` | 197.4 / 201.9 ms | 194.3 / 206.1 ms |
| 20-rep `scm` total | 0.820 / 0.895 s | 0.790 / 0.810 s |
| 20-rep `build_schedule` total | 2.430 / 2.495 s | 2.640 / 2.480 s |

All pairs overlap the run-to-run noise floor. Bit-identical: `test-interpolator`
14, `test-strategy-ff16` 47, `test-strategy-tf24` 42 (+1 skip), `test-individual`
129, `test-scm` 89, `test-strategy-ff16-reference-comparison` 16 — no tolerance
change.

**Why.** `assimilation`/`compute_rates` are hot because of their *body* work
(quadrature loop, allocation math), not call overhead; cross-TU inlining of a
large function only removes the call frame, negligible against a 15–51-point
quadrature. The ~1.3x from header-moving the *small* helpers
(`compute_competition_by_ratio`, `area_leaf`, ✪ above) worked because those are
tiny functions in the tightest per-node loop where call overhead dominated — it
does not generalise to the large Tier-3 functions. **LTO declined** (no benefit,
slower builds, CRAN-portability risk). Makevars change reverted; not committed.

**TF24 cross-check (2026-06-19).** Unlike FF16, TF24 shows a small but
consistent gain. Harness: a corrected `bench_tf24.R` (the committed script calls
the non-existent `scm_base_control()`; rewrote to use `run_scm(p1)` with
`scm_base_parameters("TF24")`, `max_patch_lifetime = 10`, `BENCH_REPS=5`).
A/B/A, clean rebuild between states:

| state | median elapsed (per SCM run) | offspring_production |
|---|---:|---|
| no-LTO baseline | 19.482 / 19.459 s | 4.730799193e-06 |
| `-flto` LTO | 18.818 / 19.066 s | 4.730799193e-06 |
| no-LTO baseline (drift re-check) | 19.462 s | 4.730799193e-06 |

≈2–3% faster, bit-identical (offspring_production unchanged to 10 sig figs). The
baseline is tight (~19.46 s) with no drift, so the effect is real but modest —
far below the ~1.3x from header-moving the small helpers. Consistent with TF24's
hot path carrying more small cross-TU helpers (leaf hydraulics solvers in
`leaf_model.cpp`) that LTO can inline.

**Implemented portably (2026-06-19): header-inline `Interpolator::eval`.** LTO
abandoned; instead captured the gain with the competition-helper move. Audit of
the TF24 leaf hot path: the only small cross-TU call in the tight loops is the
spline lookup. `Leaf::transpiration` and `E_from_Soil_to_Root_Collar` call
`transpiration_from_psi.eval()` / `root_vuln_*.eval()` per root-finder iteration,
and `Interpolator::eval` (+ private `check_active`) were defined out-of-line in
`src/interpolator.cpp` — a cross-TU barrier in front of the now-header-inline
`tk_spline(u)`. (The 3 cross-TU leaf calls from `tf24_strategy.cpp` —
`set_physiology`, `optimise_psi_stem_TF`, `find_root_collar_psi` — are large and
called once per node, not inline candidates.) Moved `eval`/`check_active` into
`interpolator.h` as inline (added `#include <plant/util.h>` for the cold
`util::stop` error paths). **Pure code move → bit-identical.**

A/B/A via the repaired `bench_tf24.R`, clean rebuild between states, BENCH_REPS=5:

| state | median elapsed / SCM run | offspring_production |
|---|---:|---|
| no-LTO baseline | 19.46 s (19.46 / 19.46 / 19.61) | 4.730799193e-06 |
| `-flto` LTO (reference) | ~18.9 s | 4.730799193e-06 |
| **header-inline `eval`** | **18.43 s (18.40 / 18.46)** | 4.730799193e-06 |

≈**1.06x (~5.5%)**, bit-identical — and it **beats full LTO** (~3%), because the
targeted move removes the exact hot barrier with zero build-flag/portability
cost. Tests: `test-interpolator` 14, `test-strategy-ff16` 47, `test-strategy-tf24`
42 (+1 skip), `test-individual` 129, `test-scm`, `test-strategy-ff16-reference-
comparison` 16, `test-leaf` 167 — all pass, no tolerance change. (`bench_tf24.R`
also repaired: it called the non-existent `scm_base_control()`; now `control()`.)
