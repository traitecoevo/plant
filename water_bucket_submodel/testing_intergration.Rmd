...
expanded_output %>%
  ...
  select(-data) %>%
  pivot_wider(names_from = focal_points, values_from = daily_profit_integrated) %>%
  ggplot(aes(x = `1`, y = `50`)) +
  geom_point() +
  geom_abline()
  ...
  # expand_grid(focal_points = c(1,2,3,50)) %>%
  mutate(point_estimate_integrals = pmap(., calc_mult_point2)) 
