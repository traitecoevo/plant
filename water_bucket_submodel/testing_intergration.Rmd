...
calc_VPH <- function(instant_of_day, VP_9, VP_15){
  ...
  if(instant_of_day <= 9){
   VP_15 + (VP_9 - VP_15)*(9+instant_of_day)/gap  
} else if(instant_of_day > 9 & instant_of_day <=15){
   VP_9 + (VP_15 - VP_9)*(instant_of_day - 9)/(15-9)
} else if (instant_of_day > 15){
   VP_15 + (VP_9 - VP_15)*(instant_of_day - 15)/gap
}
}
```

```{r}
tibble(instant_of_day = seq(0, 24, length.out = 10000), VP_9 = 2, VP_15 = 3) %>%
  rowwise() %>%
  mutate(VPH = calc_VPH(instant_of_day, VP_9, VP_15)) %>%
  ggplot(aes(x = instant_of_day, y = VPH)) +
  geom_point()
