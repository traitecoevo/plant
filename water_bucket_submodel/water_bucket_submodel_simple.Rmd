...
theta_init <- c(theta=rep(theta_input, n), runoff = 0, deep_drainage=0, cumulative_rain=0, evaporation = 0, drainage = 0, plant_water_use = rep(0,n), LAI_state = rep(1, species), a_i = rep(0, species), avg_theta = theta_input, avg_psi = calc_abs_psi(theta_input, theta_fc, theta_wp, theta_sat), top_layer_E_supply_m_h = rep(0,species), bottom_layer_E_supply_m_h = rep(0,species))
...
theta_rates <- function(t, theta, params) {
  with(as.list(c(theta, params)), {
    ...
    LAI_state_5 <- c(LAI_state1, LAI_state2)
    ...
    #multiply each E_Supply_m_h by respective root_frac giving two vectors of E_supplies

    top_layer_E_supply_m_h = E_supply_m_h * root_frac
    bottom_layer_E_supply_m_h = E_supply_m_h * (1-root_frac)

    total_top_layer_E_supply_m_h <- sum(top_layer_E_supply_m_h)
    total_bottom_layer_E_supply_m_h <- sum(bottom_layer_E_supply_m_h)
    ...
    Ev = E_bare_soil_pot_m*exp(-0.398*sum(LAI_state_5))
    ...
    outflow = drainage + Ev +total_top_layer_E_supply_m_h
    ...
    outflow = deep_drainage + total_bottom_layer_E_supply_m_h
      }
    ...
    cumulative_rain = R+S
    evaporation = Ev
    plant_water_use[i] = c(total_top_layer_E_supply_m_h, total_bottom_layer_E_supply_m_h)[i]

    }
    
    
    return(list(c(dtheta_dt, runoff, deep_drainage, cumulative_rain, evaporation, drainage, plant_water_use, dLAI_dt, a_i, avg_theta, avg_psi, top_layer_E_supply_m_h,bottom_layer_E_supply_m_h)))
...
```{r}
params <- set_params_soil(R = c(3)/365/24, mean_PPFD = 1000, evap_routine = "1")
```
...
logistic_solution2<-
  map_dfr(
    ...
    parms = .x)  %>%
      as.data.frame() %>%
      mutate(d = across(.cols = everything(), ~c(NA, diff(.x))))
  )
```
...
logistic_solution2$rainfall <- rep(c("0.5 m/yr", "3 m/yr"), each = 1001)

logistic_solution2 %>%
  filter(time != 0) %>%
  select(time, contains("LAI_state"), rainfall) %>% 
  rename(Rainfall = rainfall) %>%
  pivot_longer(contains("LAI"), names_to = "root_frac")%>%
  ggplot(aes(x = time)) +
  geom_line(aes(y=value, col =root_frac, linetype = Rainfall)) + 
  ylab("LAI") +
  xlab("Hours") + 
  theme_classic()
...
logistic_solution2 %>%
  filter(time != 0) %>%
  select(time, contains("LAI_state")) %>% 
  pivot_longer(contains("LAI"), names_to = "root_frac")%>%
  ggplot(aes(x = time)) +
  geom_line(aes(y=value, col =root_frac)) + 
  ylab("LAI") +
  xlab("Hours") + 
  theme_classic()

logistic_solution2 %>%
  mutate(experienced_theta1 = (theta1*0.1 + theta2*0.9),
         experienced_theta2 = (theta1*0.9 + theta2*0.1)) %>%
  mutate(psi1 = calc_abs_psi(theta1, theta_fc, theta_wp, theta_sat),
         psi2 = calc_abs_psi(theta2, theta_fc, theta_wp, theta_sat),
         experienced_psi1 = calc_abs_psi(experienced_theta1, theta_fc, theta_wp, theta_sat),
         experienced_psi2 = calc_abs_psi(experienced_theta2, theta_fc, theta_wp, theta_sat)) %>%
  filter(time != 0) %>%
  select(time, psi1, psi2, experienced_psi1, experienced_psi2)  %>% 
  pivot_longer(contains("psi"), names_to = "psi")%>%
  ggplot(aes(x = time)) +
  geom_line(aes(y=value, col =psi)) + 
  ylab("psi") +
  xlab("Hours") + 
  theme_classic() +
  scale_y_log10()
...
logistic_solution2 %>%
  ...
  mutate(psi1 = calc_abs_psi(theta1, theta_fc, theta_wp, theta_sat),
         ...
         experienced_psi2 = calc_abs_psi(experienced_theta2, theta_fc, theta_wp, theta_sat)) %>%
  filter(time != 0) %>%
  select(time, theta1, theta1, experienced_theta1, experienced_theta2)  %>% 
  pivot_longer(contains("theta"), names_to = "theta")%>%
  ggplot(aes(x = time)) +
  geom_line(aes(y=value, col =theta)) + 
  ylab("theta") +
  xlab("Hours") + 
  theme_classic() 
