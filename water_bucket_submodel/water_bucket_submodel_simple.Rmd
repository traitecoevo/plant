...
theta_init <- c(theta=rep(0.1, n), runoff = 0, deep_drainage=0, cumulative_rain=0, evaporation = 0, drainage = 0, plant_water_use = rep(0,n), LAI_state = 1, a_i = 0, avg_theta = 0, avg_psi = 0)
...
theta_rates <- function(t, theta, params) {
  with(as.list(c(theta, params)), {
    ...
    opt_psi_stem <- find_opt_psi_stem(psi_soil = avg_psi_MPa, k_l_max=  k_l_max, vcmax =vcmax, PPFD = PPFD, b = b, c= c, psi_crit = psi_crit, psi_crit_limit = psi_crit,atm_vpd =  VPD,ca = ca)
    E_supply_kg_m2_s <- opt_psi_stem$E_root
    

    E_supply_m_h <- E_supply_kg_m2_s * sec_2_hr * kg_2_m3 * LAI_state
    
    a_i_umol_m2_s <- opt_psi_stem$profit_root
    a_i_mol_m2_h <- a_i_umol_m2_s *sec_2_hr * umol_2_mol
    a_i <- a_i_mol_m2_h
    
    dLAI_dt <- calc_LAI(LAI = LAI_state, lma = lma, r_l = r_l, fr_l = fr_l, a_bio = a_bio, k_l = k_l, a_i = a_i_mol_m2_h)
    ...
    outflow = deep_drainage + E_supply_m_h *(z[i]/sum(z))
    ...
    return(list(c(dtheta_dt, runoff, deep_drainage, cumulative_rain, evaporation, drainage, plant_water_use, dLAI_dt, a_i, avg_theta, avg_psi)))
  })
}
...
```{r}
params <- set_params_soil(R = 1.5/365/24, mean_PPFD = 1000)
```
