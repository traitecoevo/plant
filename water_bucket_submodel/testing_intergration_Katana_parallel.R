...
packages <- c("dplyr","tidyr","ggplot2","purrr", "multidplyr")
...
my_source <- function(..., local=NULL) {
  ...
  nums <- names(tmp)[unlist(eapply(tmp, is.numeric))]
  for(x in names(tmp)) {
    assign(x, tmp[[x]], envir = parent.frame())
  }
  list(functions=funs, numerics = nums)
}

variables <- c(my_source("plant/water_bucket_submodel/R/reference_leaf_updated.R")$functions,
               my_source("plant/water_bucket_submodel/R/reference_leaf_updated.R")$numerics, 
               my_source("plant/water_bucket_submodel/R/solar_model.R")$functions, 
               my_source("plant/water_bucket_submodel/R/solar_model.R")$numerics)  


calc_daily_model_parallel <- function(params){
  cluster <- new_cluster(8)
  
  cluster_copy(cluster, names = c(variables, "calc_VPH", "calc_vpsat", "calc_temp"))
  cluster_library(cluster, c('purrr', 'tidyr'))
...
outputs <- calc_daily_model_parallel(params)
