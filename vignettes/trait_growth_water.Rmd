...
```{r}
# expand_grid(K_s = seq(1,3,length.out = 10), theta=seq(0.01,1,length.out=5), E=c(0.25,0.5,0.75,1), height = c(0.5, 2, 10, 15,20)) %>%
...
output %>%
  ...
  mutate(max_height = max(height),
         max_time = max(time)) %>%
  nest(states = c(time, height,mortality, fecundity, area_heartwood, mass_heartwood)) %>%
  filter(max_height >= to_height) %>%
  mutate(growth_rate = max_height/max_time) -> data_to_plot
```


```{r}
data_to_plot %>%
  ggplot(aes(x= K_s, y = growth_rate)) +
