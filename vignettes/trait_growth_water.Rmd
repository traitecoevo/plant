...
```{r}
expand_grid(psi = seq(0,5,length.out = 20), E = seq(0,1,length.out = 20), trait_name = "vcmax", trait_value = seq(30, 300, length.out = 9)) %>%
...
```{r}
expand_grid(psi = seq(0,5,length.out = 20), E = seq(0,1,length.out = 20), trait_name = "theta", trait_value = seq(0.00005, 0.0003, length.out = 9)) %>%
  mutate(rcp = pmap_dbl(., calculate_resource_compensation_point)) -> output
...
expand_grid(psi = seq(0,3, 1), E=c(0.25,0.5,0.75,1), height = c(0.5,1,2,5), trait_value = seq(30,200,10), trait = "vcmax") %>%
  mutate(results= pmap(., run_individual)) -> leaf_mass_area_output

expand_grid(psi = 0.1, vpd = seq(0,5,1), E=c(1), height = c(0.5,1), trait_value = seq(30,200,10), trait = "vcmax") %>%
  mutate(results= pmap(., run_individual)) -> leaf_mass_area_o
utput

expand_grid(psi = 0.1, vpd = 1, E=c(1), height = c(1), trait_value = 30, trait = "vcmax") %>%
  mutate(results= pmap(., run_individual)) -> leaf_mass_area_output
...
leaf_mass_area_output %>% 
  ...
  mutate(strategy = map(individual[[1]], ~pluck(.x, "strategy"))) %>%
  ...
  mutate(total_maintenance_sapwood_mol_yr = total_respiration_sapwood_mol_yr + total_turnover_sapwood_mol_yr) %>% View()
  ...
  unnest(state) %>%
  mutate(area_leaf = height / a_l1, 1.0 / a_l2)
...
expand_grid(psi = (1), E=c(1), height = c(2), trait_value = c(300), trait = "vcmax") %>%
  mutate(results= pmap(., run_individual)) -> leaf_mass_area_output
