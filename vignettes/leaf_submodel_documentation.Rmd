...
ff16w is a new implementation of the ff16 strategy including water dynamics. This includes an updated strategy object `ff16w`, a physiological sub-model which determines how plants assimilate carbon and transpire water dynamically in response to water availability and a soil-water submodel which takes rainfall as an input and determines how much soil moisture is available to plants.
...
-epsilon_leaf: actually a control parameter. controls accuracy of optimisation routive if newtons method is used.
-beta1: cost of sapwood per volume per unit of hydraulic damage (umol C m^{-3} sapwood)
-beta2: shape parameter for relationship between lost conductivity and sapwood repair cost (unitless). 
...
```{r}
leaf <- Leaf(vcmax= sw$vcmax_25, b = sw$b, c = sw$c, psi_crit = sw$psi_crit, epsilon_leaf = sw$epsilon_leaf, beta1 = sw$beta1, beta2 = sw$beta2, jmax_25 = sw$jmax_25, hk_s = sw$hk_s, a = sw$a, curv_fact_elec_trans = sw$curv_fact_elec_trans, curv_fact_colim = sw$curv_fact_colim)
```
