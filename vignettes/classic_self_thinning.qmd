...
# creates a suitable schedule for time steps when running the model, with many "steps" early on and fewer as the stand ages and doesn't change as rapidly

out <- run_scm_collect(stand_results)
# run the SCM model

tidy_stand <- tidy_patch(out)
# function which turns "out" into a more manageable format

stand_expand <- FF16_expand_state(tidy_stand)
# adds in more state variables from our stand output
...
patch_total <-
    ...
    mutate(
      ...
      across(c(diameter_stem, area_stem, area_leaf, height, mass_above_ground), ~.x/density, 
             .names = "{.col}_av"))
# this gives us a nice table of all the metrics gathered from the stand, including new averages calculated for leaf area, stem size (including the mean stem diameter that we need), height & biomass
```
