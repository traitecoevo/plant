...
results <- run_scm(p1, refine_schedule = TRUE)$parameters
results <- run_scm(results, collect = TRUE)
```
...
run_mypatch <- function(
  ...
  if(optimise_schedule)
    result <- run_scm(p1, refine_schedule = TRUE)$parameters
  else
    result <- p1
  
  # gather outputs at each time step
  run_scm(result, collect = TRUE)
}
```
