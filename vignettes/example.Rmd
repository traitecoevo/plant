...
```{r}
results3 <- results2 %>% expand_state()
...
results2$species %>% filter(height > 1, time> 20, time<50) %>% 
  patch_species_total() %>%
  mutate(area_stem_av = area_stem / individuals) %>% 
  ggplot(aes(area_stem_av, individuals, colour=species)) +
  geom_line() +
...
```{r}
results2$species %>% filter(cohort < 150) %>% 
  patch_species_total() %>%
  mutate(
...
results2$species %>% filter(height > 0, time> 0, time<150) %>% 
  patch_species_total() %>%
  ggplot(aes(time, individuals, colour=species)) +
  geom_line()

results2$species %>% filter(step%in% c(138, 139, 140, 141)) %>% 
...
results2$species %>% filter(height > 0, time> 30, time<31) %>% View() 
...
results2$species %>% filter(species==1) %>% plot_size_distribution_patch()   
...
```{r}
run_mypatch(traits =  trait_matrix(c(0.07, 0.24)))
...
tidy_species_data <- results2$species
...
data_combined_long <- 
  data_combined  %>% 
  select(cohort, time, step, density, height) %>% 
  pivot_longer(cols = c("density", "height"))

data_combined_long %>% 
  ggplot(aes(time, value, group=cohort)) +
  geom_line(col=1) +
...
data_combined <- 
  ...
  filter(step %in% c(100, 150, 200))

data_combined_long <- 
  data_combined  %>% 
  select(cohort, time, step, height, density, mortality, area_heartwood) %>% 
  pivot_longer(cols = c("density", "mortality", "area_heartwood"))

data_combined_long %>% 
    filter(!is.na(cohort)) %>%
  ggplot(aes(height, value, group=step)) +
  geom_line(col=1) +
  geom_point(data = data_combined_long %>% filter(is.na(cohort)), col="red") +
  xlim(c(0, 20)) +
  facet_grid(vars(name), vars(step), scales="free")
```
