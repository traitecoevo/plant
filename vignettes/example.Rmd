...
```{r}
results_tidy_expand <- results_tidy %>% expand_state_FF16()
...
Lets interpolate our results from the `run_mypatch` above for timepoints 1,5,10,1000 using the funcion `interpolate_to_times`. Note that time in units of years. 
...
Similar to above, we can interpolate across cohorts at given time to sepcific heights, suing the function `interpolate_to_heights`
