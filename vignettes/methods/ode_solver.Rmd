...
odelia::ode::Solver<patch_type> solver;   # ODE solver
```
...
evaluations. The [Adaptive vs fixed-step integration](time_stepping.html) article
works this trade-off through with a work–precision diagram. Note that
`fixed_time_step` is incompatible with the mutant-fitness replay path
