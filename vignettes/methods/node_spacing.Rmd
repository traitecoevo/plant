...
**date**: 2016, updated 2026

<!-- NB:
 This file is generated from the file `vignettes/methods/node_spacing.Rmd.orig`. Make any edits there. To regenerate
...
![plot of chunk default-times](figures/node_spacing_default-times-1.png)
...
![plot of chunk time-differences](figures/node_spacing_time-differences-1.png)
...
![plot of chunk interleaved-schedules](figures/node_spacing_interleaved-schedules-1.png)
...
![plot of chunk offspring-vs-nodes](figures/node_spacing_offspring-vs-nodes-1.png)
...
![plot of chunk insert-differences-node](figures/node_spacing_insert-differences-node-1.png)
...
![plot of chunk insert-differences-time](figures/node_spacing_insert-differences-time-1.png)
...
![plot of chunk node-contributions](figures/node_spacing_node-contributions-1.png)
...
<!-- TODO: find more general terms for 'leaf-area error' -->
...
two is exposed as `combined_node_errors` -- the signal that drives refinement:
...
combined <- scm$combined_node_errors[[1]]

plot(times_default, combined, type = "h",
     xlab = "Introduction time (years)", ylab = "Combined per-node error", las = 1)
abline(h = scm_base_control()$schedule_eps, col = "red", lty = 2)
```

![plot of chunk combined-error](figures/node_spacing_combined-error-1.png)
...
can reconstruct the per-step matrix that the solver accumulates internally,
using the still-exposed per-step accessor. Each row is one introduction step;
each column is a node:


``` r
competition_error_matrix <- function(p, env = NULL, ctrl = scm_base_control()) {
  scm <- run_scm(p, env, ctrl)   # build + run, returns the SCM object
  scm$reset()                    # rewind so we can step through it ourselves

  rows <- list()
  while (!scm$complete) {
    scm$run_next()
    rows[[length(rows) + 1]] <-
      scm$compute_competition_effect_error_by_node_for_species_i(1)
  }
...
![plot of chunk competition-error-matrix](figures/node_spacing_competition-error-matrix-1.png)
...
![plot of chunk refined-differences](figures/node_spacing_refined-differences-1.png)
