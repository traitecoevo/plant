...
```{r}
times <- c(tidy_stand$species$time[[10500]], tidy_stand$species$time[[12000]], tidy_stand$species$time[[13000]], tidy_stand$species$time[[13500]], tidy_stand$species$time[[14000]], tidy_stand$species$time[[15000]], tidy_stand$species$time[[17000]], tidy_stand$species$time[[20000]])
# give explicit rows from time column to highlight

# mark time points using `directlabels` package
...
tidy_stand$species %>%
  ...
  ggplot(aes(height, density, group=time, label = my_label)) +
  geom_line() +
  geom_dl(aes(label = my_label), method = list(dl.combine("last.points"), vjust = -0.5)) +
  xlab("height (m)") +
  ylab("density " ~(m^-2)) +
