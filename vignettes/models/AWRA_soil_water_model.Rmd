...
Prec_spline <- splinefun(x = Prec_obs$timestep, y = Prec_obs$Prec_mm_per_day)
```
...
rates_psi_FS_2021 <- function(t, Sz, params) {
  with(as.list(c(Sz, params)), {
    ...
    return(list(dSz_dt = dSz_dt, Qh = Qh, P = P))
  })
}
```
...
solve <- function(times, rates, state, method = "rk4", output = "theta") {
  ...
  deSolve::ode(
    ...
    method = method) %>% 
  as_tibble() %>% 
  pivot_longer(-c(time, Qh, P), names_to = "depth", values_to = output)
}
```
...
```{r}
  ...
  P_func_ <- function(t) {1*sin(t)+1} # Change precipitation function to a sine wave. First number is amplitude, second is the y-axis midpoint.
...
Sz_solve_lsoda <- solve(times = timesteps, rates=rates_psi_FS_2021, state=init_state, method="lsoda", output = c("Sz"))
```
...
Sz_solve_ode23 %>%
  ...
  ggplot(aes(x = time)) +
  geom_line(aes(y = as.numeric(Qh)))  -> b

Sz_solve_lsoda %>%
  # left_join(soil_moist_obs_rearranged) %>%
  ggplot(aes(x = time)) +
  geom_line(aes(y = as.numeric(P))) -> c
