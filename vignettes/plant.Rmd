...
<!-- TODO(#481): add an overview diagram contrasting plant's individual vs patch use cases -->
...
A patch is simply an area in which a collection of individuals grow and compete for resources (see [Concepts: patches](https://traitecoevo.github.io/plant/articles/demography.html#patches-of-competing-plants-size-structured-populations)). By size distribution we mean the relationship between the density of individuals and their size (see [Concepts: demography](https://traitecoevo.github.io/plant/articles/demography.html)). By default, `plant` starts with a bare/empty patch, but you can also start with an existing size distribution.

<!-- TODO(#481): add figure — example size distribution (stylised) -->
...
Plant uses a particular mathematical method to model size distributions, which assumes large populations and continuous size distribution. The dynamics are solved by integrating along so-called "characteristics" of a partial differential equation (see [Concepts: solving dynamics via the characteristic method](https://traitecoevo.github.io/plant/articles/methods/solving_dynamics.html)).
...
Below we provide some examples to get you started. More worked examples are available in the [Example analysis](https://traitecoevo.github.io/plant/articles/example_analysis.html) article.
...
We can then need to specify the particular species by specifying their traits and birth rates. Here we use `lma` but other traits are possible (see [Changing parameters](https://traitecoevo.github.io/plant/articles/parameters.html)).

```{r}
species_parameters <- expand_parameters(trait_matrix(0.0825, "lma"), params)
```

Then we run the `plant` solver (the SCM) to solve the size distribution over time, using the `run_scm` function. `run_scm` has two modes, controlled by its arguments, and we use both in turn:

- `refine_schedule = TRUE` adaptively refines the *node-introduction schedule* — the set of times at which new individuals (characteristics) are introduced into the patch. The solver adds nodes until the result meets the accuracy set by `schedule_eps` in the control object, so the resulting schedule is tuned to this particular set of species. In this mode `run_scm` returns the `SCM` object itself, from which we pull the refined `parameters`.
- `collect = TRUE` runs the solver again with that refined schedule and gathers tidied output at every ODE step, returning a list rather than the `SCM` object.

We refine first and then collect because the refined schedule is what makes the second run both accurate and efficient — there is no point gathering output until the schedule has settled.

```{r}
# 1. Refine the node-introduction schedule for the required accuracy,
#    then keep the refined parameters
species_parameters <- run_scm(species_parameters, refine_schedule = TRUE)$parameters

# 2. Rerun with that schedule, gathering tidied output at each time step
result <-
  run_scm(species_parameters, collect = TRUE)
```
...
The results is a list with a number of objects, which are explained in full in the [Patches](https://traitecoevo.github.io/plant/articles/patch.html) article, and include

* the times the size-distribution is reported (the exact times used are determined by the solver, via adaptive stepping of system. See [The node spacing algorithm](https://traitecoevo.github.io/plant/articles/methods/node_spacing.html)),
...
```{r}
species_parameters_2sp <- run_scm(species_parameters_2sp, refine_schedule = TRUE)$parameters

result <- 
  run_scm(species_parameters_2sp, collect = TRUE)
```
