...
The tidy_indivdual function puts the outputs from grow_plant_to_time into `tidy format`, as defined in the tidyverse. Examining our result, we see a matrix of our state variables at each timestep
...
result %>%
  ggplot() +
  geom_line(aes(x=time, y=height), size=1.5) +
  theme_classic()
```
...
```{r}
species_parameters_2sp <- expand_parameters(trait_matrix(0.2625, "lma"), species_parameters, mutant = FALSE)
```
