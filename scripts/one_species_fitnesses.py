...
for lma in np.logspace(np.log(0.03), np.log(0.8), 64, base=np.e)[5:-5]:
    last_seed_rain, fitness, _ = run_simmo([lma], last_seed_rain)
    seed_rains[lma] = last_seed_rain[0]
...
# import pickle
# with open("~/Data/output/tree/one_species_lmas.pickle", 'w') as f:
#     pickle.dump(np.array(robjects.r['lma.f']), f)
# with open("~/Data/output/tree/one_species_fitnesses.pickle", 'w') as f:
#     pickle.dump(fitnesses, f)
# with open("~/Data/output/tree/one_species_seed_rains.pickle", 'w') as f:
