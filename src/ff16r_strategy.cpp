...
double FF16r_Strategy::area_leaf(double height) const {
  return pow(height / a_l1, 1.0 / a_l2);
}

// [eqn 1] mass_leaf (inverse of [eqn 2])
double FF16r_Strategy::mass_leaf(double area_leaf) const {
...
double FF16r_Strategy::area_stem(double area_bark, double area_sapwood,
                            double area_heartwood) const {
  return area_bark + area_sapwood + area_heartwood;
}

double FF16r_Strategy::diameter_stem(double area_stem) const {
  return std::sqrt(4 * area_stem / M_PI);
}

// [eqn 7] Mass of (fine) roots
double FF16r_Strategy::mass_root(double area_leaf) const {
  return a_r1 * area_leaf;
}

// [eqn 8] Total mass
double FF16r_Strategy::mass_live(double mass_leaf, double mass_bark,
                           double mass_sapwood, double mass_root) const {
  return mass_leaf + mass_sapwood + mass_bark + mass_root;
}

double FF16r_Strategy::mass_total(double mass_leaf, double mass_bark,
...
double FF16r_Strategy::mass_above_ground(double mass_leaf, double mass_bark,
                            double mass_sapwood, double mass_root) const {
  return mass_leaf + mass_bark + mass_sapwood + mass_root;
}

// one-shot update of the scm variables
void FF16r_Strategy::scm_vars(const Environment& environment,
...
double FF16r_Strategy::assimilation(const Environment& environment,
                                    double height,
                                    double area_leaf,
                                    bool reuse_intervals) {
...
// where `z` is height.
double FF16r_Strategy::compute_assimilation_x(double x, double height,
...
double FF16r_Strategy::compute_assimilation_h(double z, double height,
                                     const Environment& environment) const {
  return assimilation_leaf(environment.canopy_openness(z)) * q(z, height);
}

double FF16r_Strategy::compute_assimilation_p(double p, double height,
                                     const Environment& environment) const {
...
double FF16r_Strategy::assimilation_leaf(double x) const {
  return a_p1 * x / (x + a_p2);
}

// [eqn 13] Total maintenance respiration
// NOTE: In contrast with Falster ref model, we do not normalise by a_y*a_bio.
double FF16r_Strategy::respiration(double mass_leaf, double mass_sapwood,
...
double FF16r_Strategy::respiration_leaf(double mass) const {
  return r_l * mass;
}

double FF16r_Strategy::respiration_bark(double mass) const {
  return r_b * mass;
}

double FF16r_Strategy::respiration_sapwood(double mass) const {
  return r_s * mass;
}

double FF16r_Strategy::respiration_root(double mass) const {
  return r_r * mass;
}

// [eqn 14] Total turnover
double FF16r_Strategy::turnover(double mass_leaf, double mass_bark,
                          double mass_sapwood, double mass_root) const {
...
double FF16r_Strategy::turnover_leaf(double mass) const {
  return k_l * mass;
}

double FF16r_Strategy::turnover_bark(double mass) const {
  return k_b * mass;
}

double FF16r_Strategy::turnover_sapwood(double mass) const {
  return k_s * mass;
}

double FF16r_Strategy::turnover_root(double mass) const {
  return k_r * mass;
}
...
double FF16r_Strategy::net_mass_production_dt_A(double assimilation, double respiration,
...
double FF16r_Strategy::net_mass_production_dt(const Environment& environment,
...
double FF16r_Strategy::fraction_allocation_growth(double height) const {
  return 1.0 - fraction_allocation_reproduction(height);
}

// [eqn 17] Rate of offspring production
double FF16r_Strategy::fecundity_dt(double net_mass_production_dt,
                               double fraction_allocation_reproduction) const {
  return net_mass_production_dt * fraction_allocation_reproduction /
    (omega + a_f3);
}

double FF16r_Strategy::darea_leaf_dmass_live(double area_leaf) const {
...
double FF16r_Strategy::dheight_darea_leaf(double area_leaf) const {
  return a_l1 * a_l2 * pow(area_leaf, a_l2 - 1);
}

// Mass of leaf needed for new unit area leaf, d m_s / d a_l
double FF16r_Strategy::dmass_leaf_darea_leaf(double /* area_leaf */) const {
  return lma;
}
...
double FF16r_Strategy::dmass_bark_darea_leaf(double area_leaf) const {
  return a_b1 * dmass_sapwood_darea_leaf(area_leaf);
}

// Mass of root needed for new unit area leaf, d m_r / d a_l
double FF16r_Strategy::dmass_root_darea_leaf(double /* area_leaf */) const {
  return a_r1;
}
...
double FF16r_Strategy::area_stem_dt(double area_leaf,
...
// Growth rate of basal diameter_stem per unit time
double FF16r_Strategy::diameter_stem_dt(double area_stem, double area_stem_dt) const {
  return ddiameter_stem_darea_stem(area_stem) * area_stem_dt;
}

double FF16r_Strategy::mass_root_dt(double area_leaf,
                                    double area_leaf_dt) const {
  return area_leaf_dt * dmass_root_darea_leaf(area_leaf);
}

double FF16r_Strategy::mass_live_dt(double fraction_allocation_reproduction,
...
double FF16r_Strategy::mass_total_dt(double fraction_allocation_reproduction,
...
double FF16r_Strategy::mass_above_ground_dt(double area_leaf,
...
double FF16r_Strategy::mass_heartwood_dt(double mass_sapwood) const {
  return turnover_sapwood(mass_sapwood);
}


double FF16r_Strategy::mass_live_given_height(double height) const {
...
double FF16r_Strategy::height_given_mass_leaf(double mass_leaf) const {
  return a_l1 * pow(mass_leaf / lma, a_l2);
}

double FF16r_Strategy::mortality_dt(double productivity_area,
...
double FF16r_Strategy::mortality_growth_independent_dt() const {
  return d_I;
}

double FF16r_Strategy::mortality_growth_dependent_dt(double productivity_area) const {
  return a_dG1 * exp(-a_dG2 * productivity_area);
}

// [eqn 20] Survival of seedlings during germination
double FF16r_Strategy::germination_probability(const Environment& environment) {
  const double net_mass_production_dt_ =
...
double FF16r_Strategy::area_leaf_above(double z, double height,
                                 double area_leaf) const {
  return area_leaf * Q(z, height);
}

// [eqn  9] Probability density of leaf area at height `z`
double FF16r_Strategy::q(double z, double height) const {
...
double FF16r_Strategy::Qp(double x, double height) const { // x in [0,1], unchecked.
  return pow(1 - sqrt(x), (1/eta)) * height;
}

// The aim is to find a plant height that gives the correct seed mass.
double FF16r_Strategy::height_seed(void) const {
