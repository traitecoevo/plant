...
FFdev_Strategy::FFdev_Strategy() {
  ...
  // -- r_b is defined (new) as 2*r_s, wheras the paper assumes a
...
double FFdev_Strategy::area_leaf(double height) const {
  return pow(height / a_l1, 1.0 / a_l2);
}

// [eqn 1] mass_leaf (inverse of [eqn 2])
double FFdev_Strategy::mass_leaf(double area_leaf) const {
  return area_leaf * lma;
}

// [eqn 4] area and mass of sapwood
double FFdev_Strategy::area_sapwood(double area_leaf) const {
  return area_leaf / theta;
}

double FFdev_Strategy::mass_sapwood(double area_sapwood, double height) const {
  return area_sapwood * height * eta_c * rho;
}

// [eqn 5] area and mass of bark
double FFdev_Strategy::area_bark(double area_leaf) const {
  return a_b1 * area_leaf / theta;
}

double FFdev_Strategy::mass_bark(double area_bark, double height) const {
  return area_bark * height * eta_c * rho;
}

double FFdev_Strategy::area_stem(double area_bark, double area_sapwood,
                            double area_heartwood) const {
  return area_bark + area_sapwood + area_heartwood;
}

double FFdev_Strategy::diameter_stem(double area_stem) const {
  return std::sqrt(4 * area_stem / M_PI);
}

// [eqn 7] Mass of (fine) roots
double FFdev_Strategy::mass_root(double area_leaf) const {
  return a_r1 * area_leaf;
}

// [eqn 8] Total mass
double FFdev_Strategy::mass_live(double mass_leaf, double mass_bark,
                           double mass_sapwood, double mass_root) const {
  return mass_leaf + mass_sapwood + mass_bark + mass_root;
}

double FFdev_Strategy::mass_total(double mass_leaf, double mass_bark,
...
double FFdev_Strategy::mass_above_ground(double mass_leaf, double mass_bark,
                            double mass_sapwood, double mass_root) const {
...
double FFdev_Strategy::assimilation(const Environment& environment,
                                    double height,
                                    double area_leaf,
                                    bool reuse_intervals) {
...
// where `z` is height.
double FFdev_Strategy::compute_assimilation_x(double x, double height,
...
double FFdev_Strategy::compute_assimilation_h(double z, double height,
                                     const Environment& environment) const {
  return assimilation_leaf(environment.canopy_openness(z)) * q(z, height);
}

double FFdev_Strategy::compute_assimilation_p(double p, double height,
                                     const Environment& environment) const {
...
double FFdev_Strategy::assimilation_leaf(double x) const {
  return a_p1 * x / (x + a_p2);
}

// [eqn 13] Total maintenance respiration
// NOTE: In contrast with Falster ref model, we do not normalise by a_y*a_bio.
double FFdev_Strategy::respiration(double mass_leaf, double mass_sapwood,
...
double FFdev_Strategy::respiration_leaf(double mass) const {
  return r_l * mass;
}

double FFdev_Strategy::respiration_bark(double mass) const {
  return r_b * mass;
}

double FFdev_Strategy::respiration_sapwood(double mass) const {
  return r_s * mass;
}

double FFdev_Strategy::respiration_root(double mass) const {
  return r_r * mass;
}

// [eqn 14] Total turnover
double FFdev_Strategy::turnover(double mass_leaf, double mass_bark,
                          double mass_sapwood, double mass_root) const {
...
double FFdev_Strategy::turnover_leaf(double mass) const {
  return k_l * mass;
}

double FFdev_Strategy::turnover_bark(double mass) const {
  return k_b * mass;
}

double FFdev_Strategy::turnover_sapwood(double mass) const {
  return k_s * mass;
}

double FFdev_Strategy::turnover_root(double mass) const {
  return k_r * mass;
}
...
double FFdev_Strategy::fraction_allocation_growth(double height) const {
...
double FFdev_Strategy::area_heartwood_dt(double area_leaf) const {
  return k_s * area_sapwood(area_leaf);
}

// Growth rate of bark area at base per unit time
double FFdev_Strategy::area_bark_dt(double area_leaf_dt) const {
  return a_b1 * area_leaf_dt / theta;
}

// Growth rate of stem basal area per unit time
double FFdev_Strategy::area_stem_dt(double area_leaf,
...
// Growth rate of basal diameter_stem per unit time
double FFdev_Strategy::diameter_stem_dt(double area_stem, double area_stem_dt) const {
  return ddiameter_stem_darea_stem(area_stem) * area_stem_dt;
}

double FFdev_Strategy::mass_root_dt(double area_leaf,
                                    double area_leaf_dt) const {
  return area_leaf_dt * dmass_root_darea_leaf(area_leaf);
}

double FFdev_Strategy::mass_live_dt(double fraction_allocation_reproduction,
...
double FFdev_Strategy::mass_total_dt(double fraction_allocation_reproduction,
...
double FFdev_Strategy::mass_above_ground_dt(double area_leaf,
...
double FFdev_Strategy::mass_heartwood_dt(double mass_sapwood) const {
  return turnover_sapwood(mass_sapwood);
}


double FFdev_Strategy::mass_live_given_height(double height) const {
...
double FFdev_Strategy::height_given_mass_leaf(double mass_leaf) const {
  return a_l1 * pow(mass_leaf / lma, a_l2);
}

double FFdev_Strategy::mortality_dt(double productivity_area,
...
double FFdev_Strategy::mortality_growth_independent_dt() const {
  return d_I;
}
...
double FFdev_Strategy::germination_probability(const Environment& environment) {
  const double net_mass_production_dt_ =
...
double FFdev_Strategy::area_leaf_above(double z, double height,
                                 double area_leaf) const {
  return area_leaf * Q(z, height);
}

// [eqn  9] Probability density of leaf area at height `z`
double FFdev_Strategy::q(double z, double height) const {
...
double FFdev_Strategy::Qp(double x, double height) const { // x in [0,1], unchecked.
  return pow(1 - sqrt(x), (1/eta)) * height;
}

// The aim is to find a plant height that gives the correct seed mass.
double FFdev_Strategy::height_seed(void) const {
...
void FFdev_Strategy::prepare_strategy() {
...
FFdev_Strategy::ptr make_strategy_ptr(FFdev_Strategy s) {
  s.prepare_strategy();
  return std::make_shared<FFdev_Strategy>(s);
}
