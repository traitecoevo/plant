...
double QK::integrate(util::DFunctor *f, double a, double b) {
  const double center          = 0.5 * (a + b);
  const double half_length     = 0.5 * (b - a);
  const double abs_half_length = std::abs(half_length);
  const double f_center        = eval(f, center);
  ...
  for (size_t j = 0; j < (n - 1) / 2; j++) {
    ...
    const double fval1 = eval(f, center - abscissa);
    const double fval2 = eval(f, center + abscissa);
...
// * R interface
double QK::r_integrate(util::RFunctionWrapper fun, double a, double b) {
