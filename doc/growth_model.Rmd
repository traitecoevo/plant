...
\begin{equation} k_\tm{l} = \alpha_4 \left(\frac{\phi}{\phi_0}\right)^{-\beta_4} \end{equation}

The relationship is centred around lma of $\phi_0$. Thus  $\alpha_4$ is the leaf turnover rate for a leaf with $\phi = \phi_0$.
Parameter values: $\alpha_4$ = `r tree_p[["a4"]]`, $\beta_4$ = `r tree_p[["B4"]]`, $\phi_0$ = `r tree_p[["lma"]]`
...
Description: Mortality rate for a plant with traits $x$ and height $h$ due to growth-independent factors. Set to potentially vary with wood density and height. Setting $-c_{\ud 1}>0$ means species with lower wood density have higher mortality.  Setting $\beta_6=0$ means effect varies with wood density but is independent of $h$, which is equivalent to the model in @falster_influence_2011 when $-c_{\ud 1} = 6.5E-3$. When $\beta_6 >0$ the effect of low wood density increases with height.

Determination:
\begin{equation} d_\tm{i} = c_{\ud 0}\exp\left(-c_{\ud 1}(\rho - \rho_0)h^{\beta_6} \right) \end{equation}

Parameter values: $c_{\ud 0}$ = `r tree_p[["c_d0"]]`, $c_{\ud 1}$ = `r tree_p[["c_d1"]]`, $\beta_6$ = `r tree_p[["B6"]]`
