...
test_that("Construction", {
  ...
  for (x in names(strategy_types)) {
    ...
    expect_is(p, "Plant")
    expect_is(p, sprintf("Plant<%s,FF16_Env>", x))
    expect_equal(class(p$strategy), class(s))
    expect_equal(p$strategy, s)

    coh <- Cohort(x,"FF16_Env")(s)

    expect_is(coh, "Cohort")
    expect_is(coh, sprintf("Cohort<%s,FF16_Env>", x))
    expect_equal(class(coh$plant), class(p))

    sp <- Species(x,"FF16_Env")(s)

    expect_is(sp, "Species")
    expect_is(sp, sprintf("Species<%s,FF16_Env>", x))
    expect_equal(class(sp$seed), class(coh))

    par <- Parameters(x,"FF16_Env")(strategies=list(s))
    expect_is(par, "Parameters")
    expect_is(par, sprintf("Parameters<%s,FF16_Env>", x))
    expect_equal(par$strategies[[1]], s)

    pat <- Patch(x, "FF16_Env")(par)
    expect_is(pat, "Patch")
    expect_is(pat, sprintf("Patch<%s,FF16_Env>", x))
    expect_equal(class(pat$species[[1]]), class(sp))

    scm <- SCM(x, "FF16_Env")(par)
    expect_is(scm, "SCM")
    expect_is(scm, sprintf("SCM<%s,FF16_Env>", x))
    expect_equal(class(scm$patch), class(pat))

    ## Stochastic model:
    s_sp <- StochasticSpecies(x, "FF16_Env")(s)
    expect_is(s_sp, "StochasticSpecies")
    expect_is(s_sp, sprintf("StochasticSpecies<%s,FF16_Env>", x))
    expect_equal(class(s_sp$seed), class(p))

    s_pat <- StochasticPatch(x, "FF16_Env")(par)
    expect_is(s_pat, "StochasticPatch")
    expect_is(s_pat, sprintf("StochasticPatch<%s,FF16_Env>", x))
    expect_equal(class(s_pat$species[[1]]), class(s_sp))

    s_pr <- StochasticPatchRunner(x, "FF16_Env")(par)
    expect_is(s_pr, "StochasticPatchRunner")
    expect_is(s_pr, sprintf("StochasticPatchRunner<%s,FF16_Env>", x))
