...
test_that("Construction", {
  ...
  for (x in names(strategy_types)) {
    ...
    expect_is(p, "Plant")
    expect_is(p, sprintf("Plant<%s,Env>", x))
    ...
    expect_is(coh, sprintf("Cohort<%s,Env>", x))
    expect_equal(class(coh$plant), class(p))

    sp <- Species(x,"Env")(s)

    expect_is(sp, "Species")
    expect_is(sp, sprintf("Species<%s,Env>", x))
    ...
    ## Stochastic model:
    s_sp <- StochasticSpecies(x, "Env")(s)
    expect_is(s_sp, "StochasticSpecies")
    expect_is(s_sp, sprintf("StochasticSpecies<%s,Env>", x))
    expect_equal(class(s_sp$seed), class(p))

    s_pat <- StochasticPatch(x, "Env")(par)
    expect_is(s_pat, "StochasticPatch")
    expect_is(s_pat, sprintf("StochasticPatch<%s,Env>", x))
    expect_equal(class(s_pat$species[[1]]), class(s_sp))

    s_pr <- StochasticPatchRunner(x, "Env")(par)
    expect_is(s_pr, "StochasticPatchRunner")
    expect_is(s_pr, sprintf("StochasticPatchRunner<%s,Env>", x))
