# Built from  tests/testthat/test-strategy-ff16.R on Thu Oct 29 11:14:41 2020 using the scaffolder, from the strategy:  FF16
context("Strategy-FF16ppa")
...
test_that("Defaults", {
  ...
  s <- FF16ppa_Strategy()
  expect_is(s, "FF16ppa_Strategy")

  expect_identical(sort(names(s)), keys)
  expect_identical(unclass(s)[keys], expected[keys])
})

test_that("FF16ppa collect_all_auxillary option", {

  s <- FF16ppa_Strategy()
  p <- FF16ppa_Individual(s)
  ...
  s <- FF16ppa_Strategy(collect_all_auxillary=TRUE)
...
test_that("Critical Names", {
  s <- FF16ppa_Strategy()
  my_names <- FF16ppa_Individual(s)$ode_names
  expect_identical(my_names[1:3], c("height", "mortality", "fecundity"))
})
test_that("FF16ppa_Strategy hyper-parameterisation", {
  s <- FF16ppa_Strategy()

  # lma
  lma <- c(0.1,1)
  ret <- FF16ppa_hyperpar(trait_matrix(lma, "lma"), s)
  ...
  # wood density
  rho <- c(200,300)
  ret <- FF16ppa_hyperpar(trait_matrix(rho, "rho"), s)
  ...
  ret <- FF16ppa_hyperpar(trait_matrix(narea, "narea"), s)
  ...
  ret <- FF16ppa_hyperpar(trait_matrix(omega, "omega"), s)
  ...
  ## Empty trait matrix:
  ret <- FF16ppa_hyperpar(trait_matrix(numeric(0), "lma"), s)
  expect_equal(ret, trait_matrix(numeric(0), "lma"))
})

test_that("narea calculation", {
  x <- c(1.38, 3.07, 2.94)
  p0 <- FF16ppa_Parameters()
  m <- trait_matrix(x, "hmat")
  expect_silent(sl <- strategy_list(m, p0, FF16ppa_hyperpar))

  cmp <- lapply(x, function(xi) strategy(trait_matrix(xi, "hmat"), p0, FF16ppa_hyperpar))
  expect_equal(sl, cmp)
})
