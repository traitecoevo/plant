...
test_that("FF16bg collect_all_auxillary option", {
  ...
  s <- FF16bg_Strategy(collect_all_auxillary=TRUE)
...
test_that("narea calculation", {
  ...
  expect_silent(sl <- strategy_list(m, p0, FF16bg_hyperpar))

  cmp <- lapply(x, function(xi) strategy(trait_matrix(xi, "hmat"), p0, FF16bg_hyperpar))
  expect_equal(sl, cmp)
})
...
test_that("seed rain", {
  ...
  p1 <- expand_parameters(trait_matrix(0.0825, "lma"), p0, FF16bg_hyperpar,FALSE)
  ...
  p2 <- expand_parameters(trait_matrix(0.05, "k_2"), p0, FF16bg_hyperpar, FALSE)
  p2$seed_rain <- 20
  out <- run_scm(p2)
  expect_equal(out$seed_rains, 0.007581, tolerance=1e-5)
  expect_equal(out$ode_times[c(10, 100)], c(0.000070, 5.00000), tolerance=1e-5)
})
