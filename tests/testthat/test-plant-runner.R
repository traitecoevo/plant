...
test_that("PlantRunner", {
  for (x in names(strategy_types)) {
    ...
    pr <- PlantRunner(x, "LightEnv")(p, env)
    expect_is(pr, sprintf("PlantRunner<%s,LightEnv>",x))
    expect_is(pr$plant, sprintf("Plant<%s,LightEnv>",x))
    ...
    pr <- PlantRunner(x, "LightEnv")(Plant(x, "LightEnv")(strategy_types[[x]]()), env)
    runner <- OdeRunner(x)(pr)
...
test_that("get_plant_internals_fun", {
  for (x in names(strategy_types)) {
    ...
    runner <- OdeRunner(x)(PlantRunner(x, "LightEnv")(p, env))
...
test_that("grow_plant_to_size", {
  for (x in names(strategy_types)) {
    ...
    obj <- grow_plant_to_size(Plant(x, "LightEnv")(s), heights, "height", env)
    ...
    expect_true(all(sapply(obj$plant, inherits, sprintf("Plant<%s,LightEnv>",x))))
