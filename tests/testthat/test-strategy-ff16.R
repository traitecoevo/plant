...
test_that("offspring arrival", {
  ...
  # Values re-recorded when node introduction stopped handing the stepper a
  # derivative of the pre-introduction state (see #585). The trajectory is the
  # same; the adaptive controller lands on a different accepted sequence.
  out <- run_scm(p1, env, ctrl)
  expect_equal(out$offspring_production, 16.88950, tolerance=1e-4)
