...
for (x in names(strategy_types)) {
  ...
  test_that("Basics", {
    ...
    expect_identical(sp$size_max, h0)
    expect_identical(sp$cohorts, list())
    expect_identical(sp$size_, NULL)
    expect_identical(sp$log_densities, numeric(0))
    expect_identical(sp$competition_effect, numeric(0))
    expect_identical(sp$competition_error(1.0), numeric(0))
    ...
    expect_equal(sp$sizes, seed$size)
    expect_equal(sp$log_densities, seed$log_density)
    expect_equal(sp$competition_effect, seed$competition_effect)
  ...
  ## 1: empty species (no cohorts) has no leaf area above any size:
  ...
  test_that("Leaf area sensible with three cohorts", {
    ...
    expect_identical(sp$competition_effect, cmp_competition_effect)

    cmp    <- local_error_integration(sp$sizes, cmp_competition_effect, 1.0)
    cmp_pi <- local_error_integration(sp$sizes, cmp_competition_effect, pi)

    expect_identical(sp$competition_error(), cmp)
    expect_identical(sp$competition_error(1.0), cmp)
    expect_identical(sp$competition_error(pi), cmp_pi)
