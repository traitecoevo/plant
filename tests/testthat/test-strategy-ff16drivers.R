...
test_that("FF16drivers collect_all_auxiliary option", {

  s <- FF16drivers_Strategy()
  p <- FF16drivers_Individual(s)
  ...
  s <- FF16drivers_Strategy(collect_all_auxiliary=TRUE)
...
test_that("Critical Names", {
  s <- FF16drivers_Strategy()
  my_names <- FF16drivers_Individual(s)$ode_names
  expect_identical(my_names[1:3], c("height", "mortality", "fecundity"))
})
test_that("FF16drivers_Strategy hyper-parameterisation", {
  s <- FF16drivers_Strategy()

  # lma
  lma <- c(0.1,1)
  ret <- FF16drivers_hyperpar(trait_matrix(lma, "lma"), s)
  ...
  # wood density
  rho <- c(200,300)
  ret <- FF16drivers_hyperpar(trait_matrix(rho, "rho"), s)
  ...
  ret <- FF16drivers_hyperpar(trait_matrix(narea, "narea"), s)
  ...
  ret <- FF16drivers_hyperpar(trait_matrix(omega, "omega"), s)
  ...
  ## Empty trait matrix:
  ret <- FF16drivers_hyperpar(trait_matrix(numeric(0), "lma"), s)
