...
test_that("Creation & defaults", {
  for (x in names(strategy_types)) {
    ...
    expect_is(p, sprintf("Parameters<%s,LightEnv>", x))
...
test_that("Nontrivial creation", {
  for (x in names(strategy_types)) {
    ...
    expect_error(Parameters(x, "LightEnv")(seed_rain=pi), "Incorrect length seed_rain")
    expect_error(Parameters(x, "LightEnv")(is_resident=FALSE), "Incorrect length is_resident")

    expect_error(Parameters(x, "LightEnv")(strategies=list(strategy_types[[x]](),
                                   strategy_types[[x]]()),
                                 seed_rain=pi),
...
test_that("Parameters overwrites Strategy control", {
  for (x in names(strategy_types)) {
    ...
    p <- Parameters(x, "LightEnv")(control=ctrl_p)
    ...
    ## In one shot:
    p2 <- Parameters(x, "LightEnv")(control=ctrl_p, strategies=list(s),
...
test_that("Store hyperparams", {
  for (x in names(strategy_types)) {
    p <- Parameters(x, "LightEnv")(hyperpar=hyperpar(x))
