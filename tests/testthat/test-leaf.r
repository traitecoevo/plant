...
test_that("Basic functions", {
  ...
  expect_equal(l$assim_colimited_, calc_ben_gross_one_line(psi_stem = psi_crit, psi_soil = psi_soil, atm_vpd = atm_vpd, k_l_max = leaf_specific_conductance_max, b = b, c = c, PPFD = PPFD, vcmax= vcmax, ca = ca), tolerance = 1e-03)
  
  expect_equal(l$hydraulic_cost_Sperry(psi_crit), calc_hydraulic_cost(psi_soil, psi_crit, leaf_specific_conductance_max, b, c))
  
  l$set_leaf_states_rates_from_psi_stem_analytical(psi_stem)

  expect_equal(l$assim_colimited_, calc_ben_gross_one_line(psi_stem = psi_stem, psi_soil = psi_soil, atm_vpd = atm_vpd, k_l_max = leaf_specific_conductance_max, b = b, c = c, PPFD = PPFD, vcmax= vcmax, ca = ca), tolerance = 1e-04)
  
  l$set_leaf_states_rates_from_psi_stem_analytical(psi_crit)
  
  #this value is constant following a call to set_physiology
  expect_equal(l$lambda_analytical_, calc_ben_gross_one_line(psi_stem = psi_crit, psi_soil = psi_soil, atm_vpd = atm_vpd, k_l_max = leaf_specific_conductance_max, b = b, c = c, PPFD = PPFD, vcmax= vcmax, ca = ca) /
    calc_hydraulic_cost(psi_soil, psi_crit, leaf_specific_conductance_max, b, c), tolerance = 1)
  ...
  l$set_leaf_states_rates_from_psi_stem_analytical(psi_crit)  
  l$optimise_ci_Sperry_analytical(l$ci_)  
  ...
  # expect_equal(l$profit_, opt_ci_gss(psi_soil = psi_soil, k_l_max = leaf_specific_conductance_max, vcmax = vcmax, PPFD = PPFD, b=b, c=c, psi_crit = psi_crit, atm_vpd = atm_vpd, ca =ca), tolerance = 0.1)
