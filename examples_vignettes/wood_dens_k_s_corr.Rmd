...
```{r}
tidy_function <- function(results){
  if(is.list(results)){
    result <- tidy_patch(results)%>% 
  FF16_expand_state()
...
collated_results2 %>%
  filter(!is.character(result)) %>%
  unnest(result) %>%
  unnest(parameters) %>% 
  filter(step == max(step)) %>% 
  select(step, time, species, offspring_produced_survival_weighted)
```

```{r}
collated_results %>%
  filter(!is.character(collated_results$result)) %>%
  unnest(result) %>%
  unnest(parameters) %>% 
  filter(step == max(step)) %>% 
  ...
  geom_point(aes(colour = species_p50, shape = rho)) +
  geom_line(aes(x = rainfall, y = leaf_frac, group = run, linetype = winner)) +
  scale_color_continuous(trans = "log")
```
