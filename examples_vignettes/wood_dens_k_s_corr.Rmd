...
field__focal_traits %>%
  ...
  drop_na(value) %>%
  filter(!name %in% c("ci_over_ca", "plant_height", "sapwood_specific_conductivity","seed_dry_mass", "stem_density", "water_potential_50percent_lost_conductivity", "water_potential_88percent_lost_conductivity", "stem_hydraulic_conductivity")) %>%
...
field__focal_traits %>%
  ...
  scale_x_log10() +
  scale_y_log10() + 
  geom_abline(slope = -0.1571026, intercept = -0.06260892)
```
...
run_FF16w_model <- function(inputs){
  ...
  env <- make_environment("FF16w", soil_initial_state = rep(0.1, 1), rainfall = inputs$rainfall)
  system.time(result <- build_schedule(p1, env, ctrl))
  system.time(result <- run_scm_collect(p1, env, ctrl, collect_auxiliary_variables = TRUE))
  result
}
```
...
```{r}
inputs <- expand_grid(rainfall = runif(10, 0.5, 3), p50_1 = runif(5, 0.1, 10), p50_2 = runif(5, 0.1, 10), rho = c(NA, 0.6), max_patch_lifetime = 1) %>%
  slice(sample(1:n()))
```

```{r}
system.time(result <- run_FF16w_model(inputs %>% slice(1)))
```
...
result %>%
  ...
  geom_point(aes(group = species, colour = species)) -> b
...
cowplot::plot_grid(a,b,c, nrow=2)
```
...
```{r}
results_files <- list.files("examples_vignettes/outputs/")[2:413]
