...
run_FF16w_model <- function(inputs){
  ...
  env <- make_environment("FF16w", soil_initial_state = rep(inputs$theta, 1), rainfall = inputs$rainfall)
...
```{r}
inputs <- expand_grid(rainfall = runif(10, min = 0.5, max = 3), p50_1 = runif(10, min = 0.1, max = 10), p50_2 = runif(10, min = 0.1, max = 10), initial_theta = 0.001, max_patch_lifetime = 50, schedule_eps = NA, ode_tol_abs = NA, ode_tol_rel = NA, epsilon_leaf = NA) %>%
  slice(sample(1:n())) %>%
  slice(1)
```
...
run_and_save_plant_water_model <- function(..., save = FALSE, dir = NULL){
  ...
  filename <-paste("rainfall_", round(input_parameters$rainfall,2), 
                   ...
                   "theta", input_parameters$theta, ".RDS", sep="")
