# Built from R/tf24.R on Wed Jul 22 16:42:11 2026 using the scaffolder, from the strategy: TF24
# Built from  R/TF24t.R on Mon Feb 12 09:52:27 2024 using the scaffolder, from the strategy:  TF24t
##' @title Create a TF24t Plant or Node
##' @param s A \code{\link{TF24t_Strategy}} object
##' @export
##' @rdname TF24t_Individual
##' @examples
##' pl <- TF24t_Individual()
##' pl$height
TF24t_Individual <- function(s=TF24t_Strategy()) {
  Individual("TF24t", "TF24_Env")(s)
}

##' @title Setup an a TF24t system with  default or specified parameters
...
##' @export
##' @rdname TF24t_Parameters
##' @examples
##' p1 <- TF24t_Parameters()
##' p2 <- TF24t_Parameters(max_patch_lifetime = 10.0)
TF24t_Parameters <- function(...) {
  Parameters("TF24t","TF24_Env")(...)
}

##' Generates a report on stand grown with TF24t strategy
##'
##' Builds a detailed report on stand grown with TF24t strategy, based on the template Rmd file provided.  The reports are
##' rendered as html files and saved in the specified output folder.
##'
##' @inheritParams FF16_generate_stand_report
##' @rdname TF24t_generate_stand_report
##' @return html file of the rendered report located in the specified output folder.
##' @export
TF24t_generate_stand_report <- function(results,
                                    output_file = "TF24t_report.html",
                                    overwrite = FALSE,
                                    target_ages = NA,
                                    input_file = system.file("reports", "TF24t_report.Rmd", package = "plant"),
                                    quiet = TRUE) {
  ...
  if (overwrite | !file.exists(output_file)) {
    ...
    message(sprintf("Report for TF24t stand saved at %s", output_file))
  } else {
    message(sprintf("Report for TF24t stand already exists at %s", output_file))
  }
}

##' Hyperparameters for TF24t physiological model
##' @title Hyperparameters for TF24t physiological model
...
##' @export
##' @rdname make_TF24t_hyperpar
make_TF24t_hyperpar <- function(lma_0=0.1978791,
                                B_kl1=0.4565855,
                                B_kl2=1.71,
                                rho_0=608.0,
                                B_dI1=0.01,
...
##' Hyperparameter function for TF24t physiological model
##' @title Hyperparameter function for TF24t physiological model
...
##' @export
##' @rdname TF24t_hyperpar
TF24t_hyperpar <- make_TF24t_hyperpar()

#' @export
#' @importFrom rlang .data
#' @rdname expand_state
TF24t_expand_state <- function(results) {
  ...
  for (i in seq_len(results$n_spp)) {
    ...
    allom <- TF24t_strategy_expand_allometry(s, d$height, d$area_heartwood,
                                            d$mass_heartwood)
