...
## Version: 0.2.4
## Hash: ff3c1351dc6d474be31223ce1c3ecaa0
...
SCM <- function(T) {
  type <- c(T)
  valid <- list("SCM<FF16>"="FF16", "SCM<FFdev>"="FFdev")
  constructors <- list("SCM<FF16>"=`SCM<FF16>`, "SCM<FFdev>"=`SCM<FFdev>`)
  constructors[[check_type(type, valid)]]
}
...
`SCM<FFdev>` <- function(parameters) {
  SCM___FFdev__ctor(parameters)
}
.R6_SCM___FFdev <-
  R6::R6Class(
    "SCM<FFdev>",
    ...
    public=list(
      ...
      run = function() {
        SCM___FFdev__run(self)
      },
      run_next = function() {
        SCM___FFdev__run_next(self)
      },
      reset = function() {
        SCM___FFdev__reset(self)
      },
      seed_rain = function(species_index) {
        SCM___FFdev__seed_rain(self, species_index)
      },
      seed_rain_cohort = function(species_index) {
        SCM___FFdev__seed_rain_cohort(self, species_index)
      },
      area_leaf_error = function(species_index) {
        SCM___FFdev__area_leaf_error(self, species_index)
      },
      set_cohort_schedule_times = function(times) {
        SCM___FFdev__set_cohort_schedule_times(self, times)
      }),
    active=list(
      complete = function(value) {
        if (missing(value)) {
          SCM___FFdev__complete__get(self)
        } else {
          stop("SCM<FFdev>$complete is read-only")
        }
      },
      time = function(value) {
        if (missing(value)) {
          SCM___FFdev__time__get(self)
        } else {
          stop("SCM<FFdev>$time is read-only")
        }
      },
      seed_rains = function(value) {
        if (missing(value)) {
          SCM___FFdev__seed_rains__get(self)
        } else {
          stop("SCM<FFdev>$seed_rains is read-only")
        }
      },
      parameters = function(value) {
        if (missing(value)) {
          SCM___FFdev__parameters__get(self)
        } else {
          stop("SCM<FFdev>$parameters is read-only")
        }
      },
      patch = function(value) {
        if (missing(value)) {
          SCM___FFdev__patch__get(self)
        } else {
          stop("SCM<FFdev>$patch is read-only")
        }
      },
      cohort_schedule = function(value) {
        if (missing(value)) {
          SCM___FFdev__cohort_schedule__get(self)
        } else {
          SCM___FFdev__cohort_schedule__set(self, value)
        }
      },
      ode_times = function(value) {
        if (missing(value)) {
          SCM___FFdev__ode_times__get(self)
        } else {
          stop("SCM<FFdev>$ode_times is read-only")
        }
      },
      state = function(value) {
        if (missing(value)) {
          SCM___FFdev__state__get(self)
        } else {
          stop("SCM<FFdev>$state is read-only")
        }
      },
      use_ode_times = function(value) {
        if (missing(value)) {
          SCM___FFdev__use_ode_times__get(self)
        } else {
          SCM___FFdev__use_ode_times__set(self, value)
        }
      },
      seed_rain_error = function(value) {
        if (missing(value)) {
          SCM___FFdev__seed_rain_error__get(self)
        } else {
          stop("SCM<FFdev>$seed_rain_error is read-only")
        }
      }))
