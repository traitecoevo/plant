...
##' @param env An \code{LightEnvironment} object.
...
grow_plant_to_time <- function(plant, times, env) {
  ...
  pr1 <- PlantRunner(strategy_name, "LightEnv")(plant, env)
  pr2 <- PlantRunner(strategy_name, "LightEnv")(plant, env)
...
grow_plant_bracket <- function(plant, sizes, size_name, env,
  ...
  runner <- OdeRunner(strategy_name)(PlantRunner(strategy_name, "LightEnv")(plant, env))
