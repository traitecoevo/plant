---
title: "plant: A package for modelling forest trait ecology & evolution: _Modelling the demography of plants, patches, and metacommunities_"
...
# Metacommunity dynamics \label{system-dynamics}
...
`height\_seed` within the physiological model, using the Boost
library's one-dimensional `bisect` routine
[@Schaling-2014, Eddelbuettel-2015]. The accuracy of the solution
is controlled by the parameter `plant\_seed\_tol`.
...
`ode\_tol\_rel` and `ode\_tol\_abs`.
...
`cohort\_gradient\_richardson` is `TRUE` a Richardson extrapolation
[@Stoer-2002] is used to refine the estimate, up to depth
`cohort\_gradient\_richardson`. The overall accuracy of the
derivative is controlled by `cohort\_gradient\_eps`.
...
A suitable schedule is found using the function `build\_schedule`.
...
over the allowable error threshold `schedule\_eps`. This
...
`schedule\_eps`. Decreasing `schedule\_eps` demands higher
...
be lower than, but at most equal to, `schedule\_eps`. The general idea
...
`run\_scm\_error`. We then assess how much removing the focal
cohort increases the error in these two integrations. The first
integration, performed by the function `area\_leaf\_error`, determines
...
performed by the function `seed\_rain\_error`, determines how much
...
calculated using the `local\_error\_integration` function.

For a worked example illustrating the `build\_schedule` function,
see the section `cohort\_spacing` of Appendix S3.
...
`area\_leaf\_above` in \texttt{species.h}. The main factor
...
within the `area\_leaf\_above` function is adaptively refined
via the `build\_schedule` function.
...
compared to the control parameter `environment\_light\_tol`. If
...
Library} [@Galassi-2009]; see \texttt{qag.h} for details.
If the control parameter `plant\_assimilation\_adaptive` is `TRUE`,
the integration is performed using adaptive refinement with an accuracy
controlled by the parameter `plant\_assimilation\_tol`.
...
`equilibrium\_eps` via a simple bisection algorithm (see
