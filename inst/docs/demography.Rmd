...
The notational complexity required in Eqs. \@ref(eq:size)-\@ref(eq:tildR1) potentially obscures an important point: these equations are simply the general, nonlinear solutions to
...
behaviour of the plants within the patch is given by Eqs. \@ref(eq:size)-\@ref(eq:tildR1). Integrating the dynamics over time is complicated by
...
$a_0$ to patch age $a$ is obtained via Eq. \@ref(eq:survalIndividual),
...
obtained via Eq. \@ref(eq:tild), which is solved via the IVP
\begin{equation}
...
(\#eq:boundN)
\end{equation}
Eq. \@ref(eq:boun) states that the size-density $N$ at a specific patch age $a$ is
...
The integral in Eq. \@ref(eq:boun) is solved via the IVP
\begin{equation}
...
### Approximation of size-density distribution via the characteristic method  {#approximation-of-size-density-distribution-via-the-ebt}
...
through time is the accuracy of the derivative calculation according to Eq. \@ref(eq:boun), calculated via standard finite differencing
...
be lower than, but at most equal to, `schedule_eps`. The general idea
...
see the section `cohort_spacing` of Appendix S3.
...
`area_leaf_above` in \texttt{species.h}. The main factor
...
\texttt{adaptive_interpolator.h} for details).
