---
title: "plant: A package for modelling forest trait ecology & evolution: _Description of the `FF16` physiological model_"
...
which in turn is calculated based on well-understood physiology ([Fig. 1](#fig-schematic-phys)). Various physiological parameters
...
\begin{table}[h!]
\caption{Key variables of the `FF16` physiological model. For mass ($M$), respiration ($r$), and turnover ($k$)
...
\begin{table}[h!]
\caption{Equations of the allometric growth model implemented within the `FF16` physiological model, based on functional-balance assumptions (see text for details).
