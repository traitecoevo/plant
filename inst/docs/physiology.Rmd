...
section). Tables \@ref(tab:definitions), \ref{tab:params_core}, \ref{tab:params_hyper} summarize the
...
knitr::include_graphics("figures/schematic-phys.png")
```
...
Photosynthesis per unit leaf area and respiration rates per unit leaf mass (or area) are assumed to vary with leaf nitrogen per unit area, $\nu$. The calculation of respiration rates is already described above. To calculate the average annual photosynthesis for a leaf, we integrate the instantaneous rate per unit leaf area over the annual solar trajectory, using a rectangular-hyperbolic photosynthesis light response curve, \begin{equation}
