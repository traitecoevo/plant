...
test_that("State get/set works", {
  ...
  state <- sp2$state
  expect_that(state,
              is_identical_to(matrix(c(seed$height, 0, 0, 1), 4, 1)))

  sp2$add_seeds(2)
  expect_that(sp2$state,
              is_identical_to(cbind(state, c(seed$height, 0, 0, 2))))
  state <- sp2$state
  ...
  state[2:3,] <- runif(4)
  sp2$set_ode_values(0, state[1:3,])
  expect_that(sp2$state, is_identical_to(state))

  ## Now, set to a brand new state:
  state[1,] <- sort(runif(2), decreasing=TRUE) * 10
  state[2:3,] <- runif(4)
  state[4,] <- c(3, 6)
  sp2$state <- state
  expect_that(sp2$state, is_identical_to(state))

  state <- cbind(state, c(seed$height, 0, 0, 2))
