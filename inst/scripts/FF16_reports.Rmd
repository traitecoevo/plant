...
(This is a palceholder for now. We need further wotk to rpeort on these, as they are not curently available within the parameters object.)
...
- XXX Along tradeoff curves(requires data from hyperpar?)
...
This section provides plots on size-structured population dyanmics. For the plots below we're going to use the tidy outputs format of the results.
...
- XXX notes/liunks on interpretting density
...
```
The first plot shows species with different x and y axis, enabling best visulaisation for each species
```{r}
...
- XXX Do we want to seperate out growth vs independent?
...
Note, this is the leaf area contibuted by whole individuals, against their height; so not the precise distribution of leaf area within the stand. The leaf area for each idnividual is distributed across a range of heights.
...
The first plot shows species with different y axis, enabling best visulaisation for each species
```{r}
...
p2 <- d2 %>%
  ...
  geom_text(data = d2_max, nudge_y = 0.05) +
  labs(
    title = "Leaf area index through time",
