...
pars_def <- 
 system.file("docs/FF16", "FF16-params-core.csv", package = "plant") %>% read.csv(stringsAsFactors = FALSE, header = TRUE) %>%
...
extract_birthrates <- function(strategy) {
  if (strategy$is_variable_birth_rate) {
    f <- splinefun(strategy$birth_rate_x, strategy$birth_rate_y)
  } else {
    f <- splinefun(results$time, strategy$birth_rate_y)
  }
  
  br = data.frame(time = results$time,
                  birth_rate = f(results$time))
  
  return(br)
}

map_df(p$strategies, extract_birthrates, .id = "species") %>%
  ggplot(., aes(time, birth_rate, color = species)) +
    geom_line() +
    labs(title = "Annual birth rate",
         x = "Time (years)",
         y = "Birth rate (ind. / yr / m2)",
         color = "Species")
...
p1 <- 
  ...
  labs(
    title = "Canopy openness through time",
    x = "",
