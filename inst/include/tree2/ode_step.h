...
public:
  ...
  void derivs(Problem& problem,
              const state_type& y, state_type& dydt, double t) {
    // TODO: check that problem.size() equals size...
    return ode::derivs(problem, y, dydt, t);
  }
...
// from there.
template <class Problem>
void Step<Problem>::step(Problem& problem,
                         double time, double step_size,
			 state_type &y,
			 state_type &yerr,
			 const state_type &dydt_in,
  ...
  // k2 step:
  derivs(problem, ytmp, k2, time + ah[0] * h);
  for (size_t i = 0; i < size; ++i) {
    ytmp[i] = y[i] + h * (b3[0] * k1[i] + b3[1] * k2[i]);
  }

  // k3 step:
  derivs(problem, ytmp, k3, time + ah[1] * h);
  for (size_t i = 0; i < size; ++i) {
    ytmp[i] = y[i] + h * (b4[0] * k1[i] + b4[1] * k2[i] + b4[2] * k3[i]);
  }

  // k4 step:
  derivs(problem, ytmp, k4, time + ah[2] * h);
  for (size_t i = 0; i < size; ++i) {
  ...
  // k5 step
  derivs(problem, ytmp, k5, time + ah[3] * h);
  for (size_t i = 0; i < size; ++i) {
  ...
  derivs(problem, ytmp, k6, time + ah[4] * h);
  for (size_t i = 0; i < size; ++i) {
  ...
  // Evaluate dydt_out.
  derivs(problem, y, dydt_out, time + h);
