...
public:
  ...
  virtual std::vector<std::string> aux_names() {
    std::vector<std::string> ret(
        {"competition_effect", "net_mass_production_dt", "assimilation_per_area","darea_leaf_dmass_live_", "assimilation","respiration_", "turnover_"});
  ...
  double competition_effect(double height) const { return area_leaf(height); }
  ...
  // [eqn 8] Total Mass
  double mass_live(double mass_leaf, double mass_bark, double mass_sapwood,
  ...
  double respiration(double mass_leaf, double mass_sapwood, double mass_bark,
                     double mass_root) const;
  ...
  double turnover(double mass_leaf, double mass_bark, double mass_sapwood,
                  double mass_root) const;
  ...
  double mass_root_dt(double area_leaf, double area_leaf_dt) const;
  ...
  double mortality_dt(double productivity_area,
  ...
  // to integrate over leaf distribution
  quadrature::QAG integrator;

  void initialize_integrator(bool adaptive_integration, int integration_rule,
                             int iterations, double integration_tol);
  ...
  // =  [mol CO2 / m2 / yr]  |  (39.27 = 2100 * 0.00187)  | narea *
  // photosynthesis_per_nitrogen
