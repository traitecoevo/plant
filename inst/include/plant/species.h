...
public:
  ...
  void compute_rates(const environment_type& environment, double pr_patch_survival, double birth_rate, const Control& control);
...
template <typename T, typename E>
void Species<T,E>::compute_rates(const E& environment, double pr_patch_survival, double birth_rate, const Control& control) {
  for (auto& c : cohorts) {
    c.compute_rates(environment, pr_patch_survival, control);
  }
  new_cohort.compute_initial_conditions(environment, pr_patch_survival, birth_rate, control);
}
