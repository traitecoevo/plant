...
template <> inline SEXP wrap(const plant::FF16_Strategy& x) {
  ...
  ret["state_names"] = Rcpp::wrap(x.state_names);
...
template <> inline plant::FF16_Strategy as(SEXP x) {
  ...
  // ret.state_names = Rcpp::as<decltype(retstate_names) >(xl["state_names"]);
  ret.state_names = Rcpp::as<std::vector<std::string> >(xl["state_names"]);
  return ret;
}
