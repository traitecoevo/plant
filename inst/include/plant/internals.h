...
// SPIKE (#472 scope B / #537): templated on the scalar type S so the per-plant
// state container can hold an AD active type for reverse-mode gradients. The
// `Internals` alias below pins S = double, so every existing use across the
// package keeps compiling and stays bit-identical; only AD probes instantiate
// basic_internals<ad_type>. The 0.0 / NA_REAL initialisers are wrapped in S(...)
// so they also work when S is an AD active type.
template <typename S>
