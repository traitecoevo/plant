// -*-c++-*-
#ifndef TREE_TREE_PARAMETERS_H_
#define TREE_TREE_PARAMETERS_H_
...
#include <tree/ebt_utils.h> // Unfortunately needed for setup_cohort_schedule
...
void Parameters<T>::setup_cohort_schedule() {
  ...
  if (update || cohort_schedule_times_default.empty()) {
    cohort_schedule_times_default =
      tree::cohort_schedule_times_default(cohort_schedule_max_time);
  }
