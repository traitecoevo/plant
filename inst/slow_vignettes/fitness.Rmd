...
fitness <- fitness_landscape(lma, params, ctrl = control)
...
``` {r results="hide"}
add_eq <- function(x, p, ctrl) {
  p <- expand_parameters(trait_matrix(x, "lma"), p)
  equilibrium_birth_rate(p, ctrl)
}

patches_eq <- mclapply(lma_sample, add_eq, p = params, ctrl = control, mc.cores = n_cores)
```
...
fitness_sample <- mclapply(patches_eq, 
                           function(p, lma) fitness_landscape(lma, p, ctrl = control), 
...
```{r}
lma_attr <- 0.0825
patch_eq_attr <- add_eq(lma_attr, params, ctrl = control)

fitness_attr <- fitness_landscape(lma, patch_eq_attr, ctrl = control)
...
fitness_detail <- fitness_landscape(lma_detail, patch_eq_attr, ctrl = control)
...
```{r}
patch_eq_max <- add_eq(lma_max, patch_eq_attr, ctrl = control)
fitness_max <- fitness_landscape(lma, patch_eq_max, ctrl = control)
...
```{r}
patch_eq_max_2 <- add_eq(lma_max_2, patch_eq_attr, ctrl = control)
fitness_max_2 <- fitness_landscape(lma, patch_eq_max_2, ctrl = control)
...
patch_eq_max_3 <- add_eq(lma_max_3, patch_eq_attr, ctrl = control)
fitness_max_3 <- fitness_landscape(lma, patch_eq_max_3, ctrl = control)
...
patch_eq_max_4 <- add_eq(lma_max_4, patch_eq_attr, ctrl = control)
fitness_max_4 <- fitness_landscape(lma, patch_eq_max_4, ctrl = control)
