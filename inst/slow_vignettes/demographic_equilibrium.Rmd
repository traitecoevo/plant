...
patch <- expand_parameters(trait_matrix(c(0.0825), "lma"), params,  birth_rate_list = c(1))
```
...
```{r}
lma <- c(0.0825, 0.15)
patch_2sp <- expand_parameters(trait_matrix(lma, "lma"), params,  birth_rate_list = c(17.5, 1))
...
patches <- mclapply(lma, function(x) 
  equilibrium_birth_rate(
    expand_parameters(trait_matrix(x, "lma"), params,  birth_rate_list = 1),
    ctrl = ctrl),
    mc.cores = n_cores)
```
