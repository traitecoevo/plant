...
p1 <- add_strategies(p0, traits, hyperpar = hyperpar, birth_rate = list(
...
extract_strategy <- function(strat) {
  # Biological parameters live in the nested `pars` sub-object.
  x <- strat$pars
...
extract_birthrates <- function(strategy, times) {
  ...
  df <- 
    p$strategies %>%
    map_df(extract_birthrates, times = results$steps$time, .id = "species") 
...
leaf_rates <- 
  p$strategies %>% purrr::map_df(~tibble(E, A = .x$pars$a_p1 * E / (E+ .x$pars$a_p2)),.id = "species") %>%
mutate(
...
target_ages <- target_ages[target_ages <= max(results$steps$time)]
