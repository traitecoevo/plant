...
shading <-
  results_tidy_expand$env$shading %>%
  group_by(step) %>% 
  mutate(
    # LAI at height
    LAI_above = -log(XXX_openness)/0.5,
...
d2_com <- 
  d2 %>%
  bind_rows(
    shading %>% mutate(species= "whole stand") %>%
      filter(time %in% select_closest(time, target_ages))
   )
...
```{r}
d2 <-
    shading %>% mutate(species = "shading") %>%
    filter(time %in% select_closest(time, target_ages)) %>% 
    mutate(
      time = round(time),
      LAI = -log(XXX_openness)/ 0.5      
    )
...
p1 <- 
  d2 %>%
  ggplot(aes(x = height, y = XXX_openness, label = time, group = time, col = time)) +
  geom_line() +
